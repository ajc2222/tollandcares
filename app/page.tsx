...
export default function Home(){
 ...
 const [time,setTime]=useState(0),[reduced,setReduced]=useState(false),[failed,setFailed]=useState(false),[loaded,setLoaded]=useState(false),[mobilePlayback,setMobilePlayback]=useState(false);
 ...
 useEffect(()=>{
  ...
  const el=video.current!;let frame=0,started=false;
  const tick=()=>{setTime(el.currentTime);frame=requestAnimationFrame(tick)};
  const play=()=>{if(started)return;started=true;el.playbackRate=.65;void el.play().then(()=>{setLoaded(true);cancelAnimationFrame(frame);frame=requestAnimationFrame(tick)}).catch(()=>setMobilePlayback(false))};
  el.addEventListener("canplay",play,{once:true});if(el.readyState>=3)play();
  return()=>{cancelAnimationFrame(frame);el.removeEventListener("canplay",play)};
 ...
 return <main>
  ...
  <section ref={section} className={`story ${staticMode?"static":""} ${mobilePlayback?"mobile-auto":""}`} aria-label="Care takes many forms"><div className="stage">
   <video ref={video} className="film" src="/care-journey.mp4" poster="/care-poster.jpg" muted playsInline preload="auto" autoPlay={mobilePlayback} loop={mobilePlayback} onError={()=>setFailed(true)} aria-hidden="true"/>
   <div className="shade"/>
